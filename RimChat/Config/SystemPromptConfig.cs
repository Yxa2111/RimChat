 ...
 ///</summary>
        private void InitializeMinimalDefaults()
        {
            ...
            ApiActions = new List<ApiActionConfig>
            {
                ...
                new ApiActionConfig("request_item_airdrop", "Submit an exact multi-item barter for delivery by vanilla drop pods after player confirmation. Use this for a new request; accept a pending trade card with accept_item_airdrop and its exact request_id.", "need_items (array<object>, required; faction delivers), payment_items (array<object>, required; player pays). Every row requires item (exact defName) and count (int>0). Legacy single need text remains accepted.", "Both arrays may contain multiple items and must preserve every requested item and quantity. Runtime validates all delivery defs, prices, beacon stock and deductions as one order; any invalid or unavailable row fails the whole order. Never silently reduce, substitute, or reprice a row. If a pending trade card exists, do not use this action to bypass its request_id or change its terms."),
                new ApiActionConfig("accept_item_airdrop", "Accept the current pending airdrop trade card exactly as quoted. This is the only way to accept a trade card after a previous refusal or counteroffer.", "request_id (string, REQUIRED; copy the exact request_id from [AirdropTradeCardReference])", "Use only when [AirdropTradeCardReference] is present. Copy request_id exactly; do not invent, omit, or reuse an id from an old or superseded card. This accepts the complete quoted item, quantity, payment items, and scenario without changing any term. Do not call request_item_airdrop directly for a pending trade card. If the terms need to change, wait for the player to submit a revised card."),
