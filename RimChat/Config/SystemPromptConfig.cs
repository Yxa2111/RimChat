 ...
 ///</summary>
        private void InitializeMinimalDefaults()
        {
            ...
            ApiActions = new List<ApiActionConfig>
            {
                ...
                new ApiActionConfig("request_item_airdrop", "Propose a new exact multi-item airdrop trade. The faction delivers need_items and the player pays payment_items; player confirmation is still required.", "need_items (array<object>, required), payment_items (array<object>, required), scenario (string, optional: trade/ransom/general). Every row requires item (exact defName) and count (integer 1-1000000).", "Use only when no current trade card exists. Judge the deal from the market totals and shipping values in context; runtime has no hidden price veto. Invalid definitions, unavailable stock, or an impossible deduction fail the whole order without reducing, replacing, or repricing rows."),
                new ApiActionConfig("accept_item_airdrop", "Accept the complete immutable pending airdrop quote by request_id. A previous refusal does not prevent accepting the same id later.", "request_id (string, REQUIRED; copy exactly from [AirdropTradeCardReference])", "Use only when a current trade card exists. Copy request_id exactly and do not include item, count, payment, or price parameters. Expired, cancelled, superseded, or cross-faction ids fail. Preparing, awaiting confirmation, executing, and completed states report their status without executing twice."),
            ...
            ResponseFormat = new ResponseFormatConfig
            {
                JsonTemplate = "Final reply: ordinary in-character text. Gameplay effects: call the supplied function tools.",
                ImportantRules = "1. Match the player's game language and return concise in-character text for the final reply; do not wrap it in JSON.\n2. Use only the supplied function tools for gameplay effects and read every tool result before continuing or replying.\n3. Correct invalid arguments from the concrete tool error and retry when appropriate. Multiple tool calls and multiple tool rounds are allowed.\n4. Never claim an effect succeeded before its tool result says so. awaiting_player_confirmation is not completion.\n5. Dialogue happens over the comms terminal; do not describe arriving in person or completing a hand-to-hand transfer.\n6. Judge airdrop prices from the market totals and shipping values in context; runtime has no hidden price veto.\n7. Accept a current airdrop card only with accept_item_airdrop and its exact request_id. Text-only acceptance has no gameplay effect, and a refusal does not invalidate the id.\n8. Do not add item, count, payment, or price parameters to accept_item_airdrop.\n9. Use request_info(info_type=prisoner) only when a valid ransom target id is missing."
            };
